...
const queueSymbol = Symbol('queue'),
    normlizedIndexSymbol = Symbol('nomilzedIndex'),
    findIndexForKeySymbol = Symbol('findIndexForKey');

/**
 * Allows you to handle an itterateable queue that is managed by keys and keeps order.
 *
...
class PluggableQueue {
    ...
    /**
     * Itterator that allows using the PluggableQueue inside for-of loops
     * @private
     */
    *[Symbol.iterator]() {
     ...
     * @param {int} index
     * @returns {int} index that can be safly use on the queue.
     * @private
     */
    ...
    insert(index, key, value) {
        const queue = this[queueSymbol],
            normalizedIndex = this[normlizedIndexSymbol](index);
     ...
     */
    remove(index) {
        const normalizedIndex = this[normlizedIndexSymbol](index);
    ...
    /**
     * Remoke an entity by a given key
    ...
    /**
     * Add entity to the begining of the queue
     ...
     * @throws if the exsiting key does not exist.
     */
